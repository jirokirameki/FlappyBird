...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    // SKView上にシーンが表示されたときに1度だけ呼ばれるメソッド
...
//        itemNode.position = CGPoint(x: self.frame.size.width * 0.5, y: self.groundTextureHeight + CGFloat(random_y))
    ...
    func setupGround() {
        ...
        // itemの配置用に価を保存
        self.groundTextureHeight = groundTexture.size().height
    ...
    func setupItem() {
        // itemの画像を読み込む
        let itemTexture = SKTexture(imageNamed: "apple_a")
        ...
        // 地面分 + マージンを除いた高さを上限として乱数を発生
        let random_y = (self.frame.size.height - self.groundTextureHeight - 10) * CGFloat(arc4random_uniform(10))/10.0
        
        // スプライトを作成
        item = SKSpriteNode(texture: itemTexture)
        item.position = CGPoint(x: self.frame.size.width * 0.5, y: self.groundTextureHeight + 10 + CGFloat(random_y))
        // 衝突判定用
